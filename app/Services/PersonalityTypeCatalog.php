...
/**
 * 接客タイプ診断（4軸 × 2 = 16タイプ）の解説カタログ。
 *
 * コードは [LF][CP][IO][HR] の4文字（例: LCIR）。
 * データの出典は公開診断（public/personality-test）の結果定義。
 * プロフィール表示・AIコンシェルジュ等、アプリ側の解説表示はここを正とする。
 */
class PersonalityTypeCatalog
{
    ...
    public static function get(?string $code): ?array
    {
        ...
        if (!preg_match('/^[LF][CP][IO][HR]$/', $code) || !isset(self::TYPES[$code])) {
            return null;
        }

        $axes = [];
        foreach (str_split($code) as $c) {
            $axes[] = ['code' => $c] + self::AXES[$c];
        }

        return ['code' => $code, 'axes' => $axes] + self::TYPES[$code];
    }
}
