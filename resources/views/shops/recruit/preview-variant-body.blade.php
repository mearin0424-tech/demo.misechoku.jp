...
<section id="requirements-{{ $vk }}">
    ...
    <div class="recruit-req-block">
        ...
        <div class="recruit-ref-inforow"><span class="k">給与</span><span class="v">
            @if($vk === 'trial')
                @if($vHasTrial)
                    <span style="color:#eba8c8;font-weight:800;">新規入店: {{ number_format((int) $rv['trial_hourly_wage']) }}円〜</span>
                    @if($vRegularWage > 0)
            ...
            @else
                @if($vHasHelp)
                    <span style="color:#eba8c8;font-weight:800;">{{ number_format((int) $rv['help_hourly_wage']) }}円〜</span>
                @else
                    —
                @endif
            @endif
